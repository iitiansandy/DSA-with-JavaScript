...
/*
Prob: First negative integer in every window of size k
Given an array and a positive integer k, find the first negative integer for each window(contiguous subarray) of size k. If a window does not 
...
function printFirstNegativeInteger(arr, k) {
    ...
    for(let i=k-1; i<arr.length; i++) {
        ...
        if (arr[firstNegativeIndex] < 0) {
            firstNegativeEle = arr[firstNegativeIndex];
        } else {
            firstNegativeEle = 0;
        }
        
        res.push(firstNegativeEle);
    }
    return res.join(' ');
}
...
let k = 3;
console.log(printFirstNegativeInteger(arr, k));
...
// Method 3
var maxProfit = function(prices) {
    return prices.reduce((profit, price, i, prices) => prices[i+1] > prices[i] ? profit += prices[i+1] - prices[i] : profit, 0);
};
